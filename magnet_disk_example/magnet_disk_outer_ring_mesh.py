...
outer_ring_points = [
    lm.Point(rd * 1.25, np.pi / 2 * i, ms=ks, mode='polar') for i in range(4)
]
...
outer_ring_curves = []
for i in range(len(outer_ring_points)):
    if i == len(outer_ring_points) - 1:
        outer_ring_curves.append(
            lm.Curve(outer_ring_points[i], outer_ring_points[0], origin, curve_type='arc')
        )
    else:
        outer_ring_curves.append(
            lm.Curve(outer_ring_points[i], outer_ring_points[i+1], origin, curve_type='arc')
        )

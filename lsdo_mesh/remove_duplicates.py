...
def remove_duplicate_points(points=None, point_mesh_size=None, curves=None,eps=1.e-6):
...
'''
def remove_duplicate_curves(curves=None, curve_indices=None ,curve_type=None ,surfaces=None):
    ...
    for i in range(len(curve_indices)):
        for j in range(i+1,len(curve_indices)):
            if curves[curve_indices[i,0]]==curves[curve_indices[j,0]] and curves[curve_indices[i,1]-1]==curves[curve_indices[j,1]-1] \
            and curve_type[i] == curve_type[j]:
                curve_indices_to_connect.append((i,j))
            if curves[curve_indices[i,0]]==curves[curve_indices[j,1]-1] and curves[curve_indices[i,1]-1]==curves[curve_indices[j,0]] \
             and curve_type[i] == curve_type[j]:
                curve_indices_to_connect.append((i,j))
    ...
    if dummy_curve_type == False:
        return new_curves, new_curve_indices, new_curve_types, new_surfaces, unique_surfaces
